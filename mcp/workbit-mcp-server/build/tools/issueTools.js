import { z } from 'zod';
import { makeWorkbitPatchRequest, makeWorkbitRequest, } from '../utils/workbitClient.js';
...
async function resolveMemberId(input) {
    ...
    const raw = input.assignee?.trim() || input.assigneeName?.trim() || '';
    if (!raw) {
        return { ok: true };
    }
...
async function resolveIssueId(input) {
    ...
    if (direct)
        return { ok: true, issueId: direct };
    const projectId = (input.projectId ?? '').trim();
    if (!projectId) {
        return {
            ok: false,
            error: 'issueId is required (or provide projectId + issueCode/title to resolve).',
        };
    }
    ...
    if (!rawCode && !rawTitle) {
        return {
            ok: false,
            error: 'Provide issueId, or (projectId + issueCode/title) to resolve the issue.',
        };
    }
    ...
    if (rawTitle) {
        ...
        const matches = list.filter((i) => (i.title ?? '').toLowerCase().includes(q));
        if (matches.length === 1 && matches[0]?.id?.trim()) {
            return { ok: true, issueId: matches[0].id.trim() };
        }
        if (matches.length > 1) {
...
export function registerIssueTools(server) {
    server.registerTool('getIssue', {
        ...
        try {
            ...
            const issue = await makeWorkbitRequest(`/issues/${encodeURIComponent(resolved.issueId)}`);
            return {
                content: [
                    {
                        type: 'text',
    ...
    server.registerTool('updateIssue', {
        description: 'Update issue. Use issueId, or resolve via projectId + issueCode/title. Prefer `assignee` (name/username).',
        inputSchema: {
            issueId: IssueId.optional(),
            issueCode: z.string().optional(),
            title: z.string().optional(),
    ...
    }, async ({ issueId, issueCode, title, status, assignee, assigneeId, assigneeName, projectId, description, parentIssueId, }) => {
        try {
            ...
            const result = await makeWorkbitPatchRequest(`/issues/${encodeURIComponent(resolvedIssue.issueId)}`, payload);
            return {
                content: [
                    {
                        type: 'text',
