...
async function resolveProjectId(input) {
    ...
    if (!name)
        return { ok: false, error: 'projectId (or projectName) is required.' };
...
export function registerProjectTools(server) {
    ...
    server.registerTool('createProject', {
        description: 'Create project. If workspaceId is unknown, call `getWorkspaces` first and match by name/slug (do not ask the user for ids).',
        inputSchema: {
            name: z.string(),
        ...
        try {
            ...
            if (!effectiveWorkspaceId) {
                ...
                const memberId = me && typeof me === 'object' && typeof me.id === 'string'
                    ? String(me.id).trim()
                    : '';
                if (memberId) {
                    ...
                    const match = q && ws.length > 0
                        ...
                        : undefined;
                    const wid = match && typeof match.id === 'string' ? String(match.id) : '';
            ...
            if (!effectiveWorkspaceId) {
                return {
                    content: [
                        {
                            type: 'text',
                            text: JSON.stringify({ ok: false, error: 'workspaceId is required to create a project.' }, null, 2),
                        },
                    ],
                };
            }
    ...
    server.registerTool('createProjectStatusUpdate', {
        ...
        try {
            ...
            const update = await makeWorkbitPostRequest(`/projects/${encodeURIComponent(resolved.projectId)}/status-updates`, payload);
            return {
                content: [
                    {
                        type: 'text',
    ...
    server.registerTool('getProjectStatusUpdates', {
        ...
        try {
            const payload = await makeWorkbitRequest(`/projects/${encodeURIComponent(projectId)}/status-updates`);
    ...
    server.registerTool('getProjectStatusUpdateComments', {
        ...
        try {
            const comments = await makeWorkbitRequest(`/projects/${encodeURIComponent(projectId)}/status-updates/${encodeURIComponent(updateId)}/comments`);
            return {
    ...
    server.registerTool('createProjectStatusUpdateComment', {
        ...
        try {
            const comment = await makeWorkbitPostRequest(`/projects/${encodeURIComponent(projectId)}/status-updates/${encodeURIComponent(updateId)}/comments`, { content });
            return {
    ...
    server.registerTool('createProjectDocument', {
        ...
        try {
            const doc = await makeWorkbitPostRequest(`/projects/${encodeURIComponent(projectId)}/documents`, { title, content });
            return {
                content: [{ type: 'text', text: JSON.stringify(doc, null, 2) }],
            };
        }
